...
docker buildx build --platform ${ARCH} --build-arg NEO_VERSION=${NEO_VERSION} --tag ${REPO_NAME}/neo:${GOOL_VERSION} . ${DO_PUSH} || { echo "NEO BUILD FAILED"; popd; popd; rm -rf $(pwd)/neo-build; exit 1; }
